...
function _page($$payload, $$props) {
  ...
  head($$payload, ($$payload2) => {
    $$payload2.title = `<title>Eduplay - Daftar Kuis</title>`;
  });
  $$payload.out += `<main class="grid grid-cols-1 gap-6"><h1 class="text-3xl font-bold text-slate-800">Daftar Kuis</h1> <!---->`;
  Breadcrumb($$payload, {
    children: ($$payload2) => {
      $$payload2.out += `<!---->`;
  ...
  if (data.classes.length) {
          ...
          Card_content($$payload2, {
            ...
            children: ($$payload3) => {
              if (classroom.expand?.quizzes_via_classroom.length > 0) {
                $$payload3.out += "<!--[-->";
                const each_array_1 = ensure_array_like(classroom.expand?.quizzes_via_classroom);
                $$payload3.out += `<!--[-->`;
                for (let $$index = 0, $$length2 = each_array_1.length; $$index < $$length2; $$index++) {
                  let quiz = each_array_1[$$index];
                  $$payload3.out += `<div class="flex w-full items-center justify-between gap-3"><a class="flex-1 overflow-hidden text-ellipsis whitespace-nowrap capitalize"${attr("href", "/dashboard/teachers/quizzes/" + quiz.id)}>${escape_html(quiz.name)}</a> `;
                  if (quiz.status === "open") {
                    $$payload3.out += "<!--[-->";
                    Badge($$payload3, {
                      children: ($$payload4) => {
    ...
    $$payload.out += "<!--[!-->";
    $$payload.out += `<div class="flex w-full flex-col items-center gap-3"><img src="/not-class.svg" alt="not found" class="w-[12rem]"> <h4 class="text-xl text-slate-700">Tidak ada kuis saat ini</h4></div>`;
  }
  $$payload.out += `<!--]--></main>`;
  pop();
}
...
//# sourceMappingURL=_page.svelte-cD1hmTvt.js.map
