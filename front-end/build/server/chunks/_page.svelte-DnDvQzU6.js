...
function _page($$payload, $$props) {
  ...
  const form = superForm(data.form, { validators: zodClient(signUpFormSchema) });
  const { form: formData, enhance } = form;
  let $$settled = true;
  let $$inner_payload;
  function $$render_inner($$payload2) {
    head($$payload2, ($$payload3) => {
      $$payload3.title = `<title>Eduplay - Sign Up</title>`;
    });
    $$payload2.out += `<form method="POST" class="flex w-[85%] flex-col gap-3 rounded-xl bg-card/30 px-8 py-4 shadow-lg shadow-slate-900/20 backdrop-blur-lg md:w-[50%] lg:mr-20 lg:w-[35%]"><img src="/logo.png" alt="logo app" class="mx-auto w-[12rem]"> <h2 class="mb-3 text-2xl font-bold text-slate-800">Daftar yuk</h2> `;
      ...
      children: ($$payload3) => {
        $$payload3.out += `<!---->`;
        {
          let children = function($$payload4, { props }) {
            ...
            Form_label($$payload4, {
              children: ($$payload5) => {
                $$payload5.out += `<!---->Konfirmasi Password`;
              },
              $$slots: { default: true }
            });
            $$payload4.out += `<!----> `;
    ...
    $$payload2.out += `<!----></div> `;
    Separator($$payload2, {});
    $$payload2.out += `<!----> <p class="text-slate-700">Sudah punya akun? <a href="/auth/sign-in" class="text-primary">Masuk sekarang</a></p></div></form>`;
  }
  do {
    $$settled = true;
...
//# sourceMappingURL=_page.svelte-DnDvQzU6.js.map
