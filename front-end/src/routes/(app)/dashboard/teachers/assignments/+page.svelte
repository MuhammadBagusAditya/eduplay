...
<svelte:head>
	<title>Eduplay - Daftar Tugas</title>
</svelte:head>
...
<main class="grid grid-cols-1 gap-6">
	...
	<Breadcrumb.Root>
		<Breadcrumb.List>
			...
			<Breadcrumb.Item>
				<Breadcrumb.Page>Tugas</Breadcrumb.Page>
			</Breadcrumb.Item>
		</Breadcrumb.List>
	</Breadcrumb.Root>
			...
			<Card.Root>
				...
				<Card.Content class="flex flex-col gap-3">
					{#if classroom.expand?.assignments_via_classroom.length > 0}
						{#each classroom.expand?.assignments_via_classroom as assignment}
							<div class="flex w-full items-center justify-between gap-3">
								<a
									class="flex-1 overflow-hidden text-ellipsis whitespace-nowrap capitalize"
									href={'/dashboard/teachers/assignments/' + assignment.id}
								>
									{assignment.name}
								</a>
	...
	{:else}
		<div class="flex w-full flex-col items-center gap-3">
			...
			<h4 class="text-xl text-slate-700">Tidak ada tugas saat ini</h4>
		</div>
	{/if}
</main>
